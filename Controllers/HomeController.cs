...
namespace app1.Controllers
{
    public class HomeController : Controller
    {
        ...
        public async Task<IActionResult> Index(CircleViewModel model)
        {
            if (model.rr <= 0)
            {
                ModelState.AddModelError("rr", "Bán kính không hợp lệ, vui lòng nhập lại.");
                return View(model);
            }

            var client = _clientFactory.CreateClient();
            var response = await client.GetAsync($"https://localhost:7089/api/Circle/cv_dt?rr={model.rr}");
